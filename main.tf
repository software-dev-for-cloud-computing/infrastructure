...
resource "azurerm_linux_web_app" "nodejs_app" {
  ...
  app_settings = {
    ...
    PORT                                = "80"
    NODE_ENV                            = "production"
    MONGODB_URI                         = "mongodb://${azurerm_cosmosdb_account.cosmos_account.name}:${azurerm_cosmosdb_account.cosmos_account.primary_key}@${azurerm_cosmosdb_account.cosmos_account.endpoint}/${azurerm_cosmosdb_mongo_database.mongo_database.name}?ssl=true&replicaSet=globaldb&retrywrites=false&maxIdleTimeMS=120000&appName=@${azurerm_cosmosdb_account.cosmos_account.name}@"
  }
}
